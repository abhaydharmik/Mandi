...
const Navbar = () => {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (remaining <= 0) { localStorage.removeItem("session"); logout(); return; }
    ...
    <nav className="sticky top-0 z-50 bg-white dark:bg-neutral-950 border-b border-gray-200 dark:border-white/6 backdrop-blur-md">
        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          <div className="hidden md:flex items-center gap-0.5">
            ...
            <Link to="/cart" className={`${linkClass("/cart")} flex items-center gap-1.5`}>
              ...
              Cart
              {itemCount > 0 && <span className="inline-flex items-center justify-center min-w-4.5 h-4.5 px-1 bg-orange-500 text-white text-[10px] font-bold rounded-full">{itemCount}</span>}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {timeLeft !== null && (
              <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-mono font-medium transition-colors duration-300 ${isWarning ? "bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/30 text-red-500 dark:text-red-400" : "bg-gray-50 dark:bg-white/4 border-gray-200 dark:border-white/8 text-gray-400 dark:text-neutral-500"}`}>
                <svg className={`w-3 h-3 ${isWarning ? "text-red-400" : "text-gray-400 dark:text-neutral-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                {formatTime(timeLeft)}
              </div>
            )}

            <button onClick={toggle} className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-white/20 transition-all duration-200" aria-label="Toggle theme">
              {isDark ? (
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden flex flex-col justify-center items-center w-8 h-8 sm:w-9 sm:h-9 rounded-lg hover:bg-black/5 dark:hover:bg-white/10 transition gap-1.5" aria-label="Toggle menu">
      ...
      <div className={`md:hidden border-t border-gray-100 dark:border-white/6 bg-gray-50 dark:bg-neutral-900 transition-all duration-300 overflow-hidden ${menuOpen ? "max-h-96 py-3" : "max-h-0"}`}>
        <div className="flex flex-col px-4 gap-1">
          {navLinks.map(({ to, label }) => (
            <Link key={to} to={to} onClick={() => setMenuOpen(false)} className={`px-4 py-3 rounded-lg text-sm font-medium transition-all ${isActive(to) ? "bg-black/5 dark:bg-white/10 text-gray-900 dark:text-white" : "text-gray-500 dark:text-white/50 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10"}`}>{label}</Link>
          ))}
          <Link to="/cart" onClick={() => setMenuOpen(false)} className={`flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-all ${isActive("/cart") ? "bg-black/5 dark:bg-white/10 text-gray-900 dark:text-white" : "text-gray-500 dark:text-white/50 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10"}`}>
            Cart
            {itemCount > 0 && <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 bg-orange-500 text-white text-[10px] font-bold rounded-full">{itemCount}</span>}
          </Link>

          {timeLeft !== null && (
            <div className={`mt-1 flex items-center gap-2 px-4 py-3 rounded-lg text-sm font-mono font-medium ${isWarning ? "bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400" : "bg-gray-100 dark:bg-white/4 text-gray-400 dark:text-neutral-500"}`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              Session: {formatTime(timeLeft)}
            </div>
          )}

          <button onClick={logout} className="mt-2 text-left px-4 py-3 rounded-lg text-sm font-medium text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 transition-all">↩ Logout</button>
        </div>
      </div>
    </nav>
  );
